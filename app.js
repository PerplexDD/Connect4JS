document.addEventListener('DOMContentLoaded', () => {
    ...
    (function (index) {
      ...
      squares[i].onclick = () => {
        ...
        if (squares[index + 7].classList.contains('taken')) {
          if (currentPlayer === 1 && !squares[index].classList.contains('player-two')) {
            squares[index].classList.add('taken');
            squares[index].classList.add('player-one');
            currentPlayer = 2;
            displayCurrentPlayer.innerHTML = currentPlayer;
          } else if (currentPlayer === 2 && !squares[index].classList.contains('player-one')) {
            squares[index].classList.add('taken');
  ...
  // checks on the board for a winning line of marks
  function checkBoard() {
    // Array of arrays with winning lines of marks
    const winningArrays = [
      [0, 1, 2, 3],
      [41, 40, 39, 38],
      [7, 8, 9, 10],
    ...
    for (let y = 0; y < winningArrays.length; y++) {
      ...
      if (square1.classList.contains('player-one') &&
        ...
        // if true sets player 2 the winner in result
        result.inner = "Player two wins!"
      }
    }
  }
